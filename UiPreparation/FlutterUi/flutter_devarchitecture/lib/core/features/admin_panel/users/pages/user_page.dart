...
class AdminUserPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => UserCubit()..getAll(),
      child: ExtendedBlocConsumer<UserCubit>(
        builder: (context, state) {
          if (state is BlocInitial) {
            return buildUserTable(context, []);
          }
          ...
          if (state is BlocSuccess<List<Map<String, dynamic>>>) {
            final datas = _processUserData(state.result);
  ...
  List<Map<String, dynamic>> _processUserData(
      List<Map<String, dynamic>>? rawData) {
    final data = rawData?.toList() ?? [];
    return data.map((user) {
      return {...user, "id": user["userId"]}; // Create new map with id field
    }).toList();
  }
